...
export class HocrProofreader {
    constructor(config) {
        ...
        this.layoutContainer = config.layoutContainer;
        ...
        var editorContainer = config.editorContainer;
        ...
        this.layoutSvg.addEventListener('mousemove', onMouseMove);
        this.layoutSvg.addEventListener('mouseleave', onMouseLeave);
        this.layoutContainer.addEventListener('scroll', onScroll);
    }

    setHocr(hocr, baseUrl) {
        ...
        hocrRoot.addEventListener('mousemove', (event) => {
            this.mousePosition = {container: 'editor', x: event.clientX, y: event.clientY};
            this.onHover(event.target, true);
        });
        hocrRoot.addEventListener('mouseleave', (event) => {
            this.mousePosition = null;
            this.onHover(null, true);
        });
        hocrDoc.addEventListener('scroll', (event) => {
            if (!this.mousePosition || this.mousePosition.container !== 'editor') return;
            this.onHover(hocrDoc.elementFromPoint(this.mousePosition.x, this.mousePosition.y), true);
        });
    ...
    renderNodesRecursive(node, options, parentRectsNode) {
    ...
    inheritOptions(options, parentOptions) {
    ...
    onHover(target, isEditorContainer) {
    ...
    scrollIntoViewIfNeeded(node, scrollParentNode) {
